...
- **Flyway:** location `src/main/resources/db/migration/`, naming `V{version}__{snake_case}.sql`. Tracked migrations currently run through `V6__forgot_password_reset_hardening.sql`; list the directory again before choosing the next version. Never rewrite deployed migrations. Validate PG-specific SQL (casts, vector operators) on disposable PostgreSQL + pgvector — H2 cannot validate them.
