...
public interface AccessTokenRepository extends BaseRepository<AccessToken, Long>, JpaSpecificationExecutor<AccessToken> {
    ...
    @Query("SELECT a FROM AccessToken a WHERE a.token =?1 AND a.revoked = false AND a.appUser.active=true ORDER BY a.id DESC LIMIT 1 ")
    ...
    @Query("SELECT a FROM AccessToken a WHERE a.appUser.id =?1 AND a.service=?2 AND a.revoked=?3 ORDER BY a.lastestActive DESC, a.id DESC ")
    ...
    @Query(value = "SELECT a.fcm_token FROM access_token a " +
            "WHERE a.app_user = ?1 AND a.revoked IS FALSE AND a.fcm_enable IS TRUE AND a.fcm_token IS NOT NULL", nativeQuery = true)
...
//    @Query("UPDATE AccessToken a SET a.revoked = true where a.user.id = ?1 AND a.revoked = false")
    @Query(value = "UPDATE access_token SET revoked=true WHERE appUser=?1 AND revoked is false ", nativeQuery = true)
